...
<section id="login" class="srch">
    <div class="s_tab s_tab2">
        <ul>
            <li class="on"><a href="<%=request.getContextPath() %>/searchId.do">아이디 찾기</a></li>
            <li><a href="<%=request.getContextPath() %>/searchPw.do">비밀번호 찾기</a></li>
        </ul>
    </div>
    <div id="" class="tit_wrap pt90">
        <h2>아이디 찾기</h2>
        <p>아이디를 잊으셨나요?<br>회원가입시 입력한 아래의 정보를 입력해주세요.</p>
    </div>
    <div class="login_wrap">

        <div id="" class="searchId_wrap">
			<form id="searchForm" action="<%=request.getContextPath() %>/searchId.do" method="post">
	            <div class="f_box">
	                <strong>이름 *</strong>
	                <div class="s_wrap">
	                    <input type="text" id="membername" name="membername" class="f_input" placeholder="이름을 입력해주세요" required autofocus>
	                </div>
	            </div>
	            <div class="f_box">
	                <strong>전화번호 *</strong>
	            ...
	            <button id="searchButton" class="btn1 wd100 mt50" type="submit">아이디 찾기</button>
	        </form>
        </div>
        ...
        <div id="" class="searchIdResult_wrap hide">
            ...
            <div class="btn between mt50">
                ...
                <a href="<%=request.getContextPath()%>/searchPw.do">비밀번호 찾기</a>
            </div>
        </div>
    </div>
</section>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#searchButton').click(function(event) {
            ...
            // 필요한 데이터 가져오기
            var membername = $('#membername').val().trim(); // 이름
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.memId !== '') {
                        // 일치하는 회원정보가 있을 경우
                        $('#result').html('회원님의 아이디는 <strong>' + response.memId + '</strong> 입니다.');
                    } else {
                        // 일치하는 회원정보가 없을 경우
